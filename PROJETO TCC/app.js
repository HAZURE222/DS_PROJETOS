   ...
   function handleRegister() {
     ...
     if (isMobile) {
       ...
       if (overlay) {
         ...
         termsChecked =
           overlay.querySelector('.glass-check')?.checked || false;
       }
       ...
       if (form) {
         ...
         termsChecked =
           form.querySelector('.glass-check')?.checked || false;
       }
     }
   
     // VALIDAÇÕES
     if (!nome) {
       showToast('⚠️ Digite seu nome', 'warning');
       return;
     }
   
     if (!validateEmail(email)) {
       showToast('⚠️ E-mail inválido', 'warning');
       return;
     }
   
     if (!empresa) {
       showToast('⚠️ Digite a empresa', 'warning');
       return;
     }
   
     if (senha.length < 8) {
       showToast('⚠️ Senha mínima: 8 caracteres', 'warning');
       return;
     }
   
     if (!termsChecked) {
       showToast('⚠️ Aceite os Termos de Uso', 'warning');
       return;
     }
